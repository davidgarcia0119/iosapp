...
class LoginController: UIViewController, GIDSignInUIDelegate {
    ...
    let googleLoginButton: UIButton = {
        let button = UIButton(type: .system)
        button.layer.cornerRadius = 5
        button.setTitle("Inicia Sesion con Google", for: .normal)
        button.titleLabel?.font = UIFont.systemFont(ofSize: 18)
    ...
    let dontHaveAccountButton: UIButton = {
        let button = UIButton(type: .system)
        let attributedTitle = NSMutableAttributedString(string: "No tienes una cuenta?  ", attributes: [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 16), NSAttributedString.Key.foregroundColor: UIColor.white])
        attributedTitle.append(NSAttributedString(string: "REGISTRATE", attributes: [NSAttributedString.Key.font: UIFont.boldSystemFont(ofSize: 16), NSAttributedString.Key.foregroundColor: UIColor.white]))
    ...
    func logUserIn(withEmail email: String, password: String) {
        Auth.auth().signIn(withEmail: email, password: password) { (result, error) in
            ...
            if let error = error {
                print("Fallo en inicio de sesion con error : ", error.localizedDescription)
                return
            }
