...
class SignUpController: UIViewController, GIDSignInUIDelegate {
    ...
    lazy var usernameTextField: UITextField = {
        let tf = UITextField()
        return tf.textField(withPlaceolder: "Usuario", isSecureTextEntry: false)
    }()
    
    lazy var passwordTextField: UITextField = {
        let tf = UITextField()
        return tf.textField(withPlaceolder: "Contraseña", isSecureTextEntry: true)
    }()
    ...
    lazy var dividerView: UIView = {
        ...
        let label = UILabel()
        label.text = "Otras opciones de inicio de sesion"
    ...
    let googleLoginButton: UIButton = {
        let button = UIButton(type: .system)
        button.layer.cornerRadius = 5
        button.setTitle("Inicia sesión con Google", for: .normal)
        button.titleLabel?.font = UIFont.systemFont(ofSize: 18)
    ...
    let dontHaveAccountButton: UIButton = {
        let button = UIButton(type: .system)
        let attributedTitle = NSMutableAttributedString(string: "Ya tienes una cuenta?  ", attributes: [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 16), NSAttributedString.Key.foregroundColor: UIColor.white])
        attributedTitle.append(NSAttributedString(string: "Iniciar Sesion", attributes: [NSAttributedString.Key.font: UIFont.boldSystemFont(ofSize: 16), NSAttributedString.Key.foregroundColor: UIColor.white]))
    ...
    func createUser(withEmail email: String, password: String, username: String) {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (result, error) in
            ...
            if let error = error {
                print("Error al registrar al usuario con error: ", error.localizedDescription)
                return
            }
            
            guard let uid = result?.user.uid else { return }
            
            let values = ["email": email, "usuario": username]
            
            Database.database().reference().child("usuarios").child(uid).updateChildValues(values, withCompletionBlock: { (error, ref) in
                if let error = error {
                    print("Error al actualizar los valores de la base de datos : ", error.localizedDescription)
                    return
                }
